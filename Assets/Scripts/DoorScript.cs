...
public class DoorScript : MonoBehaviour {
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        but1toPlayer = player.transform.position - but1.position + new Vector3(0, 0.8f, 0);
        but2toPlayer = player.transform.position - but2.position + new Vector3(0, 0.8f, 0);
        playerfv = camera.transform.GetComponent<Camera>().transform.forward / 5;
        ...
        if (but1toPlayer.sqrMagnitude > but2toPlayer.sqrMagnitude)
        {
            ...
            icon2.transform.LookAt(player.transform.position + new Vector3(0, 0.8f, 0));
        }
        else
        {
            ...
            icon1.transform.position = but1.position + but1toPlayer + playerfv;
            icon1.transform.LookAt(player.transform.position + new Vector3(0, 0.8f, 0));
